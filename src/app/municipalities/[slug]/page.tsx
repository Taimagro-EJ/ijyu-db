...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div style={{ marginBottom: 40 }}>
      <h2 style={{ fontSize: 15, fontWeight: 700, color: '#454034', margin: '0 0 20px', paddingBottom: 8, background: 'linear-gradient(90deg, transparent 0%, #E8E4DF 10%, #D4CCC2 30%, #E8E4DF 60%, transparent 100%)', backgroundRepeat: 'no-repeat', backgroundSize: '100% 1px', backgroundPosition: 'bottom' }}>{title}</h2>
      {children}
    </div>
  )
}
...
export default async function MunicipalityPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { data, error } = await supabase.from('municipality_overview').select('*').eq('slug', slug).single()
  ...
  return (
    <div style={{ fontFamily: "'BIZ UDPGothic', 'Noto Sans JP', sans-serif", minHeight: '100vh', background: '#F7F5F2' }}>
      <div style={{ position: 'relative', height: 320, overflow: 'hidden', background: '#454034' }}>
        {imageUrl ? (
          <Image src={imageUrl} alt={`${m.name as string}の風景`} fill style={{ objectFit: 'cover', filter: 'brightness(1.03) saturate(0.88) contrast(1.05)' }} priority />
        ) : (
        ...
        <div style={{ position: 'absolute', top: 20, left: 24 }}>
          <Link href="/" style={{ color: 'rgba(255,255,255,0.85)', fontSize: 13, textDecoration: 'none', background: 'rgba(0,0,0,0.25)', padding: '6px 14px', borderRadius: 999, backdropFilter: 'blur(4px)' }}>← 一覧に戻る</Link>
        </div>
        <div style={{ position: 'absolute', bottom: 28, left: 28, right: 28 }}>
          <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.75)', margin: '0 0 4px' }}>{m.prefecture as string} · {m.region as string}</p>
          <h1 style={{ fontSize: 36, fontWeight: 800, color: '#fff', margin: '0 0 12px', fontFamily: "'Shippori Mincho', serif", textShadow: '0 2px 12px rgba(0,0,0,0.4)' }}>{m.name as string}</h1>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '40px 24px' }}>
        ...
        <Section title="💴 生活費（推計）">
          ...
          <DataBarWithSource label="単身月額生活費（推計）" value={m.total_monthly_cost_single != null ? (m.total_monthly_cost_single as number) / 10000 : null} max={25} unit="万円" source={SOURCES.cost} invert color="#4A7C59" context={m.total_monthly_cost_single != null ? '家賃・食費・光熱費込み' : undefined} />
              ...
              <div style={{ background: '#F2F0EC', borderRadius: 12, padding: '20px' }}>
                <div style={{ marginBottom: 20 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 6 }}>
                    ...
                    <span style={{ fontSize: 36, fontWeight: 700, color: '#1A1814', fontFamily: "'DM Mono', monospace" }}>{lifestyleScore}<span style={{ fontSize: 16, color: '#9E9488' }}>/100</span></span>
                  </div>
                ...
                <div style={{ marginTop: 16, paddingTop: 16, borderTop: '1px solid #E8E4DF' }}>
                  <p style={{ fontSize: 10, color: '#9E9488', lineHeight: 1.6 }}>💡 施設データ（OpenStreetMap）をもとに、カバレッジ40%・充実度30%・コスパ30%の3層構造で算出。全国527市町村で標準化。</p>
                </div>
              </div>
            </div>
          </Section>
        ...
        <div style={{ margin: '32px 0', padding: '24px 28px', background: 'linear-gradient(135deg, #F2F0EC 0%, #F0DBC8 100%)', borderRadius: 16, border: '1px solid #E8E4DF' }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 16 }}>
            ...
            <div>
              <p style={{ fontWeight: 700, color: '#454034', fontSize: 16, margin: '0 0 6px', fontFamily: "'Shippori Mincho', serif" }}>{m.name as string}への移住、あなたの条件で試算してみませんか？</p>
              <p style={{ fontSize: 13, color: '#6B6457', margin: '0 0 14px' }}>527市町村のデータから、年収・家族構成・希望条件に合う移住先をAIが提案します。</p>
              <Link href="/chat" style={{ display: 'inline-block', padding: '10px 20px', background: '#D46B3A', color: '#fff', borderRadius: 8, fontSize: 13, fontWeight: 600, textDecoration: 'none' }}>AIに無料相談する</Link>
            </div>
          </div>
        </div>
