...
export default async function MunicipalityPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const hasFacilityData = m.convenience_count != null || (m as any).cafe_total != null || (m as any).hospital_count != null
        ...
        <Section title="🚅 アクセス">
          ...
          <div style={{ marginTop: 16, padding: '12px 16px', background: '#F7F5F2', borderRadius: 10, border: '1px solid #E8E4DF' }}>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              ...
              <a href={`https://www.google.com/maps/dir/${encodeURIComponent((m.name as string) + (m.prefecture as string))}/博多駅`} target="_blank" rel="noopener noreferrer" style={{ fontSize: 12, padding: '5px 12px', background: '#fff', border: '1px solid #E8E4DF', borderRadius: 999, color: '#3D5A80', textDecoration: 'none', whiteSpace: 'nowrap' }}>→ 博多駅</a>
              <a href={`https://www.google.com/maps/dir/${encodeURIComponent((m.name as string) + (m.prefecture as string))}/札幌駅`} target="_blank" rel="noopener noreferrer" style={{ fontSize: 12, padding: '5px 12px', background: '#fff', border: '1px solid #E8E4DF', borderRadius: 999, color: '#3D5A80', textDecoration: 'none', whiteSpace: 'nowrap' }}>→ 札幌駅</a>
            </div>
          </div>
        </Section>
        ...
        {hasFacilityData && (
          ...
          <Section title="🧘 ウェルビーイング">
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 12 }}>
              ...
              <FacilityCard municipalityId={municipalityId} municipalityName={m.name as string} category="cafe" label="カフェ" expectedCount={((m as any).cafe_total ?? 0) as number} value={(m as any).cafe_total != null ? `${(m as any).cafe_total}軒` : '-'} source={SOURCES.facility} />
            </div>
          </Section>
          {/* E. おでかけと暮らし */}
